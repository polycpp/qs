...
- `.tmp/upstream/qs/lib/stringify.js`: source of stringify option defaults and deferred JS-only surfaces such as custom filters, sort callbacks, Date, Buffer, Symbol, and BigInt handling.
...
- Upstream accepts JavaScript values that do not map cleanly to `JsonValue`, including Date, Buffer, Symbol, BigInt, `undefined`, functions, and arrays used as filters.
...
- polycpp core paths inspected: `<polycpp-checkout>/include/polycpp/core/json.hpp`, `<polycpp-checkout>/include/polycpp/core/error.hpp`, `<polycpp-checkout>/include/polycpp/core/number.hpp`, plus `<polycpp-checkout>/include/polycpp/{http,https,net,tls,io,stream,buffer,events}` for Node parity surfaces.
...
- polycpp core types/functions rejected: `polycpp::http::Headers`, request/response, URL, Buffer, streams, events, timers, sockets, TLS, crypto, filesystem, and Promise primitives are not part of the qs public contract.
...
- Date/time interop policy: no public `Date` input is supported in v0; upstream Date serialization is deferred because `JsonValue` does not carry Date instances.
...
- new local abstractions introduced: `ParseOptions`, `StringifyOptions`, `Duplicates`, `ArrayFormat`, `Format`, and private helpers for percent encoding/decoding, merge, compaction, and key segmentation; these are package-specific and justified because base polycpp has JSON and numeric primitives but not qs semantics.
...
- callback APIs: none in target package; analyzer callback signals came from installed helper dependencies and are not shipped by this C++ port.
...
- Buffer and binary APIs: upstream stringify treats Buffer as a primitive string-like value; v0 `JsonValue` does not carry Buffer, so Buffer-specific stringify is omitted and documented as a divergence.
...
- unsupported Node-specific APIs and audit reason: CommonJS packaging, dynamic option type validation, JS prototype/`plainObjects` behavior, Date, Buffer, Symbol, BigInt, custom encoder/decoder/filter/sort callbacks, and runtime dependency objects are either JS-runtime-only or intentionally adapted to typed C++ options and `JsonValue`.
...
- Deferred upstream parity work: parse `charsetSentinel`, `interpretNumericEntities`, and custom `decoder`.
- Deferred upstream parity work: stringify `charsetSentinel`, custom `encoder`, `formatter`, `filter`, `sort`, `serializeDate`, deprecated `indices`, and Date/Buffer adapters once the public value model can carry or adapt those values.
...
- unsupported APIs: upstream JS runtime callbacks/custom encoders/decoders/filters/sorters, Date/Buffer/Symbol/BigInt/undefined value handling, CommonJS/browser package artifacts, and `qs.formats` object identity.
...
- polycpp modules to use: `polycpp/core/json.hpp`, `polycpp/core/error.hpp`, and `polycpp/core/number.hpp`.
- missing polycpp primitives: none for the supported qs scope; transport, listener, stream, Buffer, and event primitives exist in base polycpp but are not needed by qs.
