...
- Parse `charsetSentinel` and `interpretNumericEntities` are not implemented yet; callers choose `ParseOptions::charset` directly.
- Parse custom `decoder` is not implemented because v0 exposes a typed C++ parser without JavaScript callback hooks.
- Stringify custom `encoder`, `formatter`, `filter`, `sort`, and `serializeDate` are not implemented yet.
- Stringify `charsetSentinel` output is not implemented yet.
- Deprecated stringify `indices` compatibility option is not implemented; callers use `ArrayFormat::indices` or `ArrayFormat::repeat`.
- A public `formats` object equivalent to `qs.formats` is not exposed; `Format` covers the supported selector.
- Upstream-compatible Date and Buffer stringify support is deferred until the public data model can carry those value types or adapters are chosen.
...
- Stringify accepts `JsonValue`; JavaScript values such as function, Symbol, BigInt, `undefined`, and arbitrary object instances are outside v0.
...
- Upstream runtime behavior for Date, Buffer, Symbol, BigInt, functions, and `undefined` is unsupported in v0 because those values are not representable in `JsonValue`.
